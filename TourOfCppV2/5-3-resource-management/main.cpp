...
std::vector<int> moveVector2(int const n) {
  ...
  std::vector<int> v2 = v;
  std::cout << "vector data address of newly assugned before return : "
            << v2.data() << std::endl;
  return v2;
}
...
int main(void) {
  ...
  std::cout << "Address of returned vector.data from moveVector is " << v.data()
            << std::endl;
  v = moveVector2(2);
  std::cout << "Address of returned vector.data from moveVector2 is "
            << v.data() << std::endl;
  v = moveVector3(2);
  std::cout << "Address of returned vector.data from moveVector3 is "
            << v.data() << std::endl;
  return 0;
}
